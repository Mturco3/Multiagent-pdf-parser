...
def generate_notes(
    ...
    with tempfile.TemporaryDirectory(prefix="lecture_notes_") as temp_dir:
        ...
        pipeline = Pipeline(temp_pdf, clear_cache=False, cache_root=cache_root, progress_callback=progress_callback)
...
st.caption("Upload lecture slides to generate Markdown notes. The PDF content is sent to the configured model provider and temporary upload files are deleted after processing.")
...
if uploaded_file is not None and st.button("Generate Notes", type="primary"):
    ...
    def show_progress(stage: str, current: int | None, total: int | None, detail: str):
    ...
    try:
        generated_document, generated_filename = generate_notes(uploaded_file, progress_callback=show_progress)
        ...
        progress_status.update(label="Notes generated", state="complete", expanded=False)
    except DailyQuotaExceededError as error:
        progress_status.update(label="Model quota exhausted", state="error", expanded=True)
        st.error(f"Model quota exhausted: {error}")
    except Exception as error:
        progress_status.update(label="Pipeline failed", state="error", expanded=True)
        st.exception(error)
